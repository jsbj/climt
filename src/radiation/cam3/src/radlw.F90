...
!
   ...
   do k=ntoplw,pver-1
      do i=1,ncol
         bk2(i) = (abstot_3d(i,k,pver,lchnk) + abstot_3d(i,k,pverp,lchnk))*0.5
         bk1(i) = bk2(i)
...
!CSD$ PARALLEL DO PRIVATE( bk1, bk2 )
      do k=pverp,ntoplw,-1
         if (k == km) then
            ...
            do i=1,ncol
               bk2(i) = (abstot_3d(i,k,km-1,lchnk) + abstot_3d(i,k,km,lchnk))*0.5
               bk1(i) = bk2(i)
            end do
         end if
         do i=1,ncol
...
!
   ...
   call outfld('FUL     ',ful*1.e-3,pcols,lchnk)
   call outfld('FDL     ',fdl*1.e-3,pcols,lchnk)
   call outfld('FULC    ',fsul*1.e-3,pcols,lchnk)
   call outfld('FDLC    ',fsdl*1.e-3,pcols,lchnk)
#endif
   ...
   do k=ntoplw,pver
      do i=1,ncol
         qrl(i,k) = (ful(i,k) - fdl(i,k) - ful(i,k+1) + fdl(i,k+1))* &
                     1.E-4*gravit_cgs/((pint(i,k) - pint(i,k+1)))
      end do
   end do
...
!
   gravit_cgs  =  100.*gravit
   stebol_cgs  =  1.e3*stebol
