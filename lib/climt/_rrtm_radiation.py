...
def driver(*args):
    ...
    SW_BANDS = [[2600,3250],[3250,4000],[4000,4650],[4650,5150],[5150,6150],[6150,7700],[7700,8050],[8050,12850],
        [12850,16000],[16000,22650],[22650,29000],[29000,38000],[38000,50000],[820,2600]]
    LW_BANDS = [[10,250],[250,500],[500,630],[630,700],[700,820],[820,980],[980,1080],[1080,1180],[1180,1390],
        [1390,1480],[1480,1800],[1800,2080],[2080,2250],[2250,2380],[2380,2600],[2600,3000]]
    ...
    if climt_inputs['Tbound'] == None: climt_inputs['Tbound'] = climt_inputs['T']
    ...
    if 'h2o' in climt_inputs and not (climt_inputs['h2o'] == None):
        ...
        h2o_concentration = [[(1.e-3 * q * (461.5/287.04))[0][0] for q in climt_inputs['q']]]
    ...
    rrtm_inputs = [
        ...
        ['o3vmr',  [[o3[0][0] * 0.603428 and 0. for o3 in climt_inputs['o3']]]], # convert from kg/kg to volume mixing ratio using molecular weight of dry air / ozone
        ['co2vmr',  [[co2 * 1.e-6 for co2 in climt_inputs['co2']]]],
        ['ch4vmr',  [[ch4 * 1.e-6 and 0. for ch4 in climt_inputs['ch4']]]],
        ['n2ovmr',  [[n2o * 1.e-6 and 0. for n2o in climt_inputs['n2o']]]],
        ['o2vmr',  [[.21 for o2 in climt_inputs['o2']]]],
        ['cfc11vmr',  [[cfc11 * 1.e-6 and 0. for cfc11 in climt_inputs['cfc11']]]],
        ['cfc12vmr',  [[cfc12 * 1.e-6 and 0. for cfc12 in climt_inputs['cfc12']]]],
        ['cfc22vmr',  [[cfc22 * 1.e-6 and 0. for cfc22 in climt_inputs['cfc22']]]],
